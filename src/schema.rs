...
use crate::model::{ResetUptime, SetPower, Subsystem};
...
graphql_object!(Subsystem: Context as "Subsystem" |&self| {
    ...
    field average_radiation_count() -> FieldResult<i32> as "Average radiation count" {
        Ok(self.avg_radiation_count()?)
    }

    field radiation_count(counter_id: i32) -> FieldResult<i32> as "Radiation count of counter_id" {
        Ok(self.radiation_count(counter_id)?)
    }
});
